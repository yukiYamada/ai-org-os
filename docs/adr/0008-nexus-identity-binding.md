...
nx.send_dispatch(from_mind="bob",   to_mind="carol", ...) # AuthorizationError

nx.read_inbox(mind_name="alice")   # OK
nx.read_inbox(mind_name="bob")     # AuthorizationError

nx.ack_dispatch(mind_name="alice", msg_id=...)   # OK
nx.ack_dispatch(mind_name="bob",   msg_id=...)   # AuthorizationError
```

`AuthorizationError` は **ai-org-os 専用例外**（`storage.py` で定義）。built-in `PermissionError` を使わない理由は **Codex P2 PR #27 follow-up** を参照：fs 操作（`write_text` / `read_text` / `rename`）が出す OS-level `PermissionError` と意味的に被るのを避けるため。これにより呼び出し側は「認可拒否」と「インフラエラー」を区別できる。

### MCP wiring 側

`AuthorizationError` は `nexus.py` の `call_tool` で捕捉され、`{"ok": false, "error": "...", "code": "forbidden"}` を返す。MCP クライアント（Mind 内 Claude）には JSON として届く。`PermissionError` などの fs-level エラーは `except Exception` 側に落ちて、`code: "forbidden"` は付かない。

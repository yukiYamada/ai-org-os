...
Judgment 呼び出しが成功したら (= status="ok"、`fallback-*` ではない)、読んだメッセージ全件を `ack_dispatch` する:
...
ack タイミングを「読んだ直後」ではなく「Judgment 後」にする理由: Judgment 呼び出しが失敗 (network / API rate limit 等) した場合に、次 cycle で **同じ reply を再度読んで** judgment 入力にできる (= at-least-once 配送)。Judgment は冪等な観察関数なので、同じ入力で重複呼び出し可。fallback-* に倒れた場合は ack しない = 次 cycle で reprocess。
...
- 大量 reply burst で fallback-* 状態が続くと inbox が貯まる (ack しないため)。これは Judgment 復旧時に一気に解消するので一時的問題

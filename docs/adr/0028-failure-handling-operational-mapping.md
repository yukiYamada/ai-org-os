...
**用例**: 当初は auto-kill (timeout streak max) のみ L1 critical 通知だったが、Phase 5g.B #172 chunk 3 で **cost threshold 超過** (`mind_loop.cost_warn`) を severity=warning として L1 に乗せた。channel は同じ `logs/notify.jsonl`、severity field でフィルタする想定。今後 L3 (`AI_ORG_OS_NOTIFY_CMD`) が実装されたら同じ event がそのまま push される。

...
L1 + L2 は機械強制 (A)、L3 は利用者選択 (C)。L1/L2 は Phase 5f Step 4 で、**L3 は Phase 5g.B #173 で実装** (= 本書執筆時点では実装済)。

**L3 fork 仕様** (Phase 5g.B #173):
- env `AI_ORG_OS_NOTIFY_CMD` に shell command 1 行を指定 (= bash -c で解釈)
- `_mindloop_notify` が L1 file write 後に command を fork、**JSON entry 1 行を stdin で渡す**
- env `AI_ORG_OS_NOTIFY_CMD_TIMEOUT` (default 5s) で hang ガード (= GNU `timeout --kill-after=2` で wrap)
- timeout / exit non-zero / fork 失敗 等は全て silent (= Realm を止めない、ADR-0013 §1 F3)
- timeout 不在環境 (= MSYS minimal 等) では未保護 fork + stderr warning

**用例**: 当初は auto-kill (timeout streak max) のみ L1 critical 通知だったが、Phase 5g.B #172 chunk 3 で **cost threshold 超過** (`mind_loop.cost_warn`) を severity=warning として L1 に乗せた。channel は同じ `logs/notify.jsonl`、severity field でフィルタする想定。L3 (`AI_ORG_OS_NOTIFY_CMD`) は同じ event をそのまま外部へ push する。

**L3 cmd の例**:
```bash
# Slack incoming webhook (= jq で message 抽出)
export AI_ORG_OS_NOTIFY_CMD='jq -r "{text: \"[\(.severity)] \(.actor): \(.message)\"}" | curl -s -X POST -H "Content-Type: application/json" -d @- "${SLACK_WEBHOOK_URL}"'

# GitHub issue 自動起票 (= gh CLI)
export AI_ORG_OS_NOTIFY_CMD='jq -r "[\(.severity)] \(.actor) \(.event): \(.message)" | xargs -I {} gh issue create --title "{}" --body "Auto-filed from mind-loop"'

# 単純に別 file に append
export AI_ORG_OS_NOTIFY_CMD='cat >> /var/log/ai-org-os-critical.log'
```

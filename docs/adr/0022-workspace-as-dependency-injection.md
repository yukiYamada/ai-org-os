...
### 3. 物理レイアウト: 「Mindspace 直下に `work/` subdir = git worktree」モデル (mode=worktree 時)

`spawn-mind.sh --workspace <name>` を渡したとき:

1. Workspace template を 2 層 overlay で解決
2. `vcs: git` + `mode: worktree` なら:
   - 通常通り Mindspace dir (`$AI_ORG_OS_HOME/minds/<mind_name>/`) を作成 (既存挙動)
   - その中に **`work/` subdir** を作り、ここを worktree にする:
     `git -C <repo> worktree add <Mindspace>/work -b <branch_prefix>/<mind_name>`
   - **Mindspace 直下**は従来通り Mind メタ (`CLAUDE.md` / `.mcp.json` / `.mind-meta.md`) を保持。`work/` subdir のみが repo の worktree
3. Mind は `<Mindspace>/work/` に cd して `git status` / `git diff` / `git commit` を叩くと自分の作業 branch が見える
4. `kill-mind.sh` は Mindspace の rm 前に `git -C <repo> worktree remove --force <Mindspace>/work` を実行 (登録解除)

#### 設計上の選択 (Phase 5d-2 で確定)

**当初案** (本 ADR Initial Accepted 時): Mindspace **そのもの** を worktree とし、`.git/info/exclude` か Mindspace 内 `.mind/` subdir で Mind メタを worktree の git 管理から外す。

**問題発覚** (2026-05-27 Phase 5d-2 着手時): target repo が **既存の `CLAUDE.md`** を root に持っている場合 (ai-org-os repo 自身がそう)、worktree がそれを check out すると Mind の Persona `CLAUDE.md` と **物理的に衝突**する。`.git/info/exclude` は untracked file の話で、tracked file の上書きは別問題。

**確定版** (Phase 5d-2): Mindspace 直下に **`work/` subdir** を作り、ここを worktree にする。Mindspace root と worktree が別 dir なので:

- target repo の任意の file 名と Mind メタが衝突しない
- 既存の Mindspace 直下レイアウト (CLAUDE.md / .mcp.json / .mind-meta.md) が完全互換
- Mind は cd の使い分けで「Persona / MCP config を見る (Mindspace root)」「コード作業 (work/)」を区別

ADR の意図 (= branch 単位の隔離、Mindspace 不可侵の精神を VCS 層に拡張) は **そのまま保たれる**: worktree の branch は依然 1 Mind に閉じ、他 Mind は別 branch で別 worktree。隔離は branch レベルで物理保証される。

この設計の利点:

- **隔離が物理保証される**: 各 Mind は別 branch / 別 worktree なので、他 Mind の作業中ファイルを覗けない (ADR-0014 Mindspace 不可侵の精神を VCS 層に拡張)
- **Mindspace の位置が変わらない**: ADR-0018 (`$AI_ORG_OS_HOME/minds/<name>/`) の合意を破らない
- **既存 Persona / Nexus 接続が温存される**: Mindspace root の CLAUDE.md / .mcp.json はそのまま
- **target repo の file 名と衝突しない**: `work/` subdir が worktree なので、repo 側に CLAUDE.md があっても問題なし
- **kill = worktree remove + Mindspace rm**: Mind の死とともに `work/` の未 commit 変更が消えるのは **意図的** (組織として「未保存の作業は引き継がない」= 「同期は Dispatch / PR 経由で行え」を強制)

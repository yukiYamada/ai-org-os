...
**Superseded by [ADR-0005](./0005-phase-3-mcp-direct-with-nexus.md)** — 2026-05-22

> 本 ADR は当初「ファイル経由 + poll」を推奨案として書かれたが、壁打ちセッション
> の結果、Phase 3 は **MCP 直行（Nexus 経由）** で実装することに方針変更された。
> 採用案は ADR-0005 を参照。
>
> ただし本 ADR の論点比較（ファイル経由 / メッセージキュー / MCP / stdin-stdout 等のトレードオフ）は
> 将来の判断材料として残置する。

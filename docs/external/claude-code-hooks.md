...
仕様は公式ドキュメント（`https://docs.claude.com/en/docs/claude-code/hooks` → `https://code.claude.com/docs/en/hooks` にリダイレクト、2026-05-23 時点）と、ローカルの claude-team リポジトリ（`C:\Users\kokoro068\git\claude-team`）の実装を突き合わせて作成した。バージョン依存の挙動は §4 に隔離してある。
...
`Edit` / `Write` の `tool_input.file_path` を `AI_ORG_OS_MIND_NAME` と照合し、他 Mind の Mindspace を編集しようとしたら deny する。claude-team の `block-tools.sh`（`C:\Users\kokoro068\git\claude-team\plugins\supervisor-mode\scripts\block-tools.sh`）が allowlist 方式の実装例として参考になる。
...
- 外部リポジトリ（参照のみ、ai-org-os は依存しない）:
  - `C:\Users\kokoro068\git\claude-team\plugins\supervisor-mode\scripts\block-tools.sh` — `Edit` / `Write` の file_path allowlist 実装例。
  - `C:\Users\kokoro068\git\claude-team\plugins\supervisor-mode\scripts\block-bash-commands.sh` — `Bash` の固定文字列ブロックリスト実装例。
  - `C:\Users\kokoro068\git\claude-team\plugins\supervisor-mode\hooks\hooks.json` — plugin 形式での `.claude/settings.json` 同等の宣言例。

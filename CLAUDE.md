...
- **Phase 5g = 「framework foundation v2 + 箱庭 v2」** (2026-06-05 設計): 2 Epic 並走:
  - **5g.A: Framework foundation v2** (#164): primitive 完成度を上げて publish 可能に。**5/5 merged (2026-06-08〜09)**: (1) composition (#166, PR #186)、(2) schema (#167, PR #185)、(3) test harness (#168, PR #188)、(5) versioning (#170, PR #189)、(4) **Kind diversity (#169, PR #191)** が完了。`persona.py` + `persona_contract.py` で C 層 4 entity の検証 + frontmatter `mixins: [...]` 共通 section 集約 (`templates/persona-mixins/`) + declarative ルールの machine-readable 化 + drift detection。`version.py` で `runtime/VERSION = 1.0.0` SSOT + C 層 manifest の `framework_version: ">=X.Y"` constraint で起動時 mismatch WARN、`CHANGELOG.md` で SemVer + bump discipline + v1.0→v1.1 migration テンプレ。`registry.py` の Kind に `runtime` field 追加で **4 Kind** (= claude / deterministic / api / human) を表現可能に、spawn-mind.sh + mind-loop.sh が runtime ごとに dispatch (claude path 既存、deterministic 完全実装 = Persona の ```bash``` block を body.sh に抽出 + 1 cycle で実行、api/human は spec only の exit 4)、reference Persona `watcher.md` 同梱。tracking #164 close 可
  - **5g.B: 箱庭 v2** (#165): 物理基盤 + 観察強化。**完了 (2026-06-22)**: observability 軸全実装 + Security P2 完了。cost meter (#172→#177/178/179)、L3 notify (#173→#180)、Realm health view (#174→#181)、Mindspace persistence (#171→#182)、chain viz (#175→#183)、per-cycle tool breakdown (#193→#203)。Security P2 batch (#195-199→#202): observe.py symlink skip、kill-mind /proc fallback を attacker-restrictive に、storage.py reserved name 二重防御、snapshot.py .tmp 判定厳密化、kill-mind orphan sweep argv fallback 削除。`observe.py` に `--cost / --status / --chain / --tool-breakdown` が揃い、kill-mind --preserve / spawn-mind --restore-from で state 引き継ぎ可、`AI_ORG_OS_NOTIFY_CMD` で外部 push 通知可。残 (独立タスク): **#108 WSL/Linux 移行**、**#134 cycle outlier RCA**。tracking #165 close

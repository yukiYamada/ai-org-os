...
- **Phase 5g = 「framework foundation v2 + 箱庭 v2」** (2026-06-05 設計): 2 Epic 並走:
  - **5g.A: Framework foundation v2** (#164): primitive 完成度を上げて publish 可能に。5 子 issue: (1) Persona composition #166、(2) C 層 schema #167、(3) Org test harness #168、(4) Kind diversity #169、(5) Framework versioning #170。推奨順: schema → composition → test → versioning → Kind。**4/5 merged (2026-06-08)**: (1) composition (#166, PR #186)、(2) schema (#167, PR #185)、(3) **test harness (#168, PR #188)**、(5) **versioning (#170, PR #189)** が完了。`persona.py` が C 層 4 entity の検証 + frontmatter `mixins: [...]` で共通 section 集約 (例: `templates/persona-mixins/mindspace-info.md`)、`persona_contract.py` で declarative ルールの machine-readable 化 + drift detection (`forbidden_ops` cross-check)、`version.py` で `runtime/VERSION = 1.0.0` を SSOT 化 + C 層 manifest の `framework_version: ">=X.Y"` constraint で起動時 mismatch WARN、`CHANGELOG.md` で SemVer + bump discipline + v1.0→v1.1 migration テンプレ。残 1 件: **#169 Kind diversity** (= api / deterministic / human 3 Kind 設計、multi-day)

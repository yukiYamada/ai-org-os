...
### Realm Inbox (人間 → Mind、Phase 5b-2 / #75 / ADR-0017)

人間が `runtime/issues/inbox/` に投入した Issue を Mind が自分で取り込むための tool。
Inbox Pillar (`runtime/pillars/inbox/`) を cross-pillar import で wrap している。

| tool | 役割 |
|---|---|
| `read_pending_issues()` | Realm Inbox の未処理 Issue 一覧 (frontmatter + body) |
| `claim_issue(mind_name, issue_id)` | Issue を archive に移し、`claimed_by`/`claimed_at` を frontmatter に追記。identity binding で `mind_name` は呼び出し元 Mind と一致が要求される |

`read_inbox` (Mind 同士の Dispatch) と `read_pending_issues` (人間からの Issue) は **別物**。前者は自分宛、後者は共有キュー（誰でも読める / 先勝ち claim）。
